      ...
      <div className="min-h-screen bg-[#FFF9F2] pb-28">
        ...
        <div className="space-y-5 px-4 pt-4">
          <div className="flex justify-center gap-2">
            {[1, 2, 3].map((step) => (
              <span
                key={step}
                className={cn(
                  'type-label flex h-8 w-8 items-center justify-center rounded-full',
                  step === 1
          ...
          {selectedMeals.length > 0 && (
            <section>
              <p className="mb-2 type-label text-[#1A1A1A]">Your Order</p>
              <div className="space-y-2">
                {selectedMeals.map(({ meal, quantity }) => (
                  <div key={meal.id} className="flex items-center gap-3">
                    ...
                    <span className="flex-1 type-body font-semibold text-[#1A1A1A]">{meal.name}</span>
                    <span className="type-body font-semibold text-[#F07B2A]">x{quantity}</span>
                  </div>
                ))}
              </div>
            </section>
          ...
          <section>
            <label className="mb-1.5 block type-label text-[#1A1A1A]">Your Name</label>
            <input
              value={name}
          ...
          <section>
            <label className="mb-1.5 block type-label text-[#1A1A1A]">Phone Number</label>
            <input
              value={phone}
          ...
          <section>
            <p className="mb-2 type-label text-[#1A1A1A]">Delivery location</p>
            <div className="flex gap-3 rounded-xl border border-[#E8E3DA] bg-[#FDF8F3] p-4">
              <MapPin size={22} className="shrink-0 text-[#F07B2A]" />
              <div>
                <p className={cn(TypeClass.bodyMd, 'font-semibold text-[#1A1A1A]')}>
                  {selectedLocation?.name ?? 'Brampton Distribution Hub'}
                </p>
                <p className={cn(TypeClass.caption, 'text-[#6B7280]')}>{deliveryAddress}</p>
                <p className={cn(TypeClass.caption, 'mt-0.5 font-semibold text-[#F07B2A]')}>
          ...
          <section>
            <p className="type-label text-[#1A1A1A]">Serving Size</p>
            <p className={cn(TypeClass.caption, 'mb-2 text-[#6B7280]')}>
            ...
            <div className="flex gap-3">
              {SERVING_SIZES.map((size) => (
                <button
                  ...
                  className={cn(
                    'type-label flex h-12 w-12 items-center justify-center rounded-full',
                    servingSize === size
                      ? 'bg-[#F07B2A] text-white'
          ...
          <section>
            <p className="mb-2 type-label text-[#1A1A1A]">Delivery window</p>
            <div className="grid grid-cols-3 gap-2">
              {DELIVERY_WINDOWS.map((window) => (
                <button
                  ...
                  className={cn(
                    'type-label-sm rounded-xl py-2.5 font-semibold',
          ...
          <section>
            <p className="mb-2 type-label text-[#1A1A1A]">Delivery preference</p>
            <div className="grid grid-cols-2 gap-2">
              <button
                type="button"
                onClick={() => setDeliveryPreference('leave_at_door')}
                className={cn(
                  'type-label-sm flex items-center justify-center gap-2 rounded-xl py-3 font-semibold',
          ...
          <section>
            <p className="type-label text-[#1A1A1A]">
          ...
          <section>
            <p className="type-label text-[#1A1A1A]">Donate (optional)</p>
            ...
            <div className="relative">
              ...
              <input
                ...
                className={cn('type-body-md w-full rounded-xl border border-[#E8E3DA] bg-white py-3 pl-8 pr-4 text-[#1A1A1A] outline-none focus:border-[#F07B2A]')}
              />
            </div>
          </section>
        </div>
        ...
        <div className="fixed bottom-0 left-1/2 w-full max-w-[430px] -translate-x-1/2 px-4 pb-8 pt-2">
          <button
            ...
            className={cn(TypeClass.btn, 'flex w-full items-center justify-center gap-2 rounded-[28px] bg-[#F07B2A] py-4 text-white shadow-[0_6px_14px_rgba(240,123,42,0.35)] disabled:opacity-60 active:scale-[0.99]')}
          >
